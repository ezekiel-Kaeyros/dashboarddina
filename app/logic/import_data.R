...
# file.data <- paste(path_data, "/dina_data.json", sep="")
...
for (i in 1:nrow(data)){
  if (!is.null(data$dateRangeState[[i]][1])){
    startDate = c(startDate, strsplit(data$dateRangeState, ",")[[i]][1])
    endDate =  c(endDate, strsplit(data$dateRangeState, ",")[[i]][2])
  } else {
...
#"""""""""""""""""""""""""""""""""""""""""""""
data$identity <- gsub("Ich berichte im Namen der betroffenen Person",
                             "Im Namen der Betroffenen", data$identity)
data$identity <- gsub("Eine Organisation/Institution",
                             "Organisation/Institution", data$identity)


#"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
data$locationOnline <- ifelse(data$locationOnline == "JA" , "Online", "Das wirkliche Leben")


#""""""""""""""""""""""""""""""""""""""""""""""""""""""
data$haveYouReported <- gsub("ich habe den Fall bei der polizei angezeigt",
                             "Polizei angezeigt", data$haveYouReported)
data$haveYouReported <- gsub("Ich habe eine Beratungsstelle aufgesucht",
                             "Beratungsstelle aufgesucht", data$haveYouReported)
data$haveYouReported<- gsub("Ich habe den vorfall bei einer anderen Meldestelle gemeldet :",
                            "Andere Meldestelle informiert", data$haveYouReported)
data$haveYouReported <- gsub("Anderes, und zwar",
                             "Anderes", data$haveYouReported)
data$haveYouReported <- gsub("Nein",
                             "Hat nichts gemacht", data$haveYouReported)
...
#""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
data$otherFormOfDisc <- gsub("Anderes und zwar:", "Anderes", data$otherFormOfDisc)
#data$otherFormOfDisc <- gsub("Nein", "Did nothing", data$otherFormOfDisc)


#""""""""""""""""""""""""""""""""""""""""""""""""""""""""
data$manifestationOfDiscrimination <- gsub("Anderes und zwar:", "Anderes", data$manifestationOfDiscrimination)
...
key_var <- c("identity", "gender",
             "otherFormOfDisc", "age", "locationOnline","organizationType",
             "numberOfEmployees", "manifestationOfDiscrimination","haveYouReported")
